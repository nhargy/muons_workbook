...
from utils.functions import linear, gaussian, hist_to_scatter
...
class Run:
    ...
    def __init__(self):
        self.data  = [] # [(timestamp, angle, hits), ...]
        self.rates = []
        self.total_time = 0
        self.event_num = 0
    ...
    def add_run(self, runpath):
        ...
        for segment in range(1, segment_number+1):
            ...
            sys.stdout.write(f"\r{segment}")  # Overwrites previous output
            sys.stdout.flush() 
    ...
    def get_rate(self):
        rate  = np.round(self.event_num / self.total_time , 5)
        drate = np.round(np.sqrt(self.event_num) / self.total_time, 5)
        return rate, drate
